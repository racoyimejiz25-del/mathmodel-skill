...
本文件定义 mathmodel-skill 在不同 agentic harness 下运行时的**统一行为约定**。Codex 可通过原生 skill / plugin 形态发现本仓库；skill 主体内容 (`SKILL.md`, `stage_NN.md`, `competitions/*`) 保持 harness-agnostic，仅在以下方面有差异。
...
- [ ] 所有"选 X" 决策点都呈现编号选项
- [ ] decision_log.json schema 完全一致 (含兼容字段)
